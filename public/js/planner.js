document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (plannerContainer) {
    ...
    let components = [];
    let rows = [{ id: Date.now(), componentId: "main_page", note: "" }];
    ...
    try {
      ...
      if (response.ok) {
        ...
        rowsContainer.innerHTML =
          '<p class="text-red-500 p-4">Failed to load components. Please ensure server is running.</p>';
      }
    } catch (error) {
      console.error("Error fetching planner components:", error);
      rowsContainer.innerHTML =
        '<p class="text-red-500 p-4">Error fetching planner components.</p>';
    }
      ...
      rowsContainer.innerHTML = rows
        .map((row, index) => {
          const selectedComponent =
            components.find((c) => c.id === row.componentId) || components[0];
